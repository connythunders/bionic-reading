...
Större datamängder ligger bredvid som JSON (`so-quiz-data.json`, `arsenal-data.json`, `skolresultat-gy-data.json`) eller som delade skript i `js/`.
...
Tre GitHub Actions sköter sig själva:

| Workflow | Vad den gör |
|---|---|
| `partierna-nyheter.yml` | Uppdaterar `partierna-data.json` varje timme |
| `arsenal-news.yml` | Uppdaterar `arsenal-data.json` varje timme |
