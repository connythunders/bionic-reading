...
import AmnesValjare from "@/components/AmnesValjare";
...
const LS_NYCKEL = "amnesomraden-grundskola:senaste";
...
export default function Page() {
  ...
  async function hitta(e?: React.FormEvent) {
    ...
    if (valda.length === 0) {
      setFel("Välj minst ett ämne att väva in.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/generera", {
        ...
        body: JSON.stringify({ ankaramne, tema: tema.trim(), amnen: valda }),
      });
      ...
      sparaSenaste({ ankaramne, tema: tema.trim(), amnen: valda });
    } catch (err) {
    ...
    <main className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      <header className="mb-8">
        ...
        <p className="mt-3 max-w-2xl text-dampad">
          Skriv in ett tema och välj ett eller flera ämnen att väva in. Du får
          3 förslag på ämnesövergripande arbetsområden för högstadiet,
          förankrade i det centrala innehållet i Lgr22.
        </p>
      </header>
      ...
      >
        <div className="grid gap-5 sm:grid-cols-[200px_1fr]">
          ...
          <label className="flex flex-col gap-1.5">
            ...
            <input
              ...
              placeholder="t.ex. hållbar utveckling"
        ...
        <div className="mt-6">
          ...
          <AmnesValjare valda={valda} onToggle={toggleAmne} />
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          {valda.length > 0 && (
            <span className="text-sm text-dampad">
              {valda.length} ämne{valda.length === 1 ? "" : "n"} valda:{" "}
              {valda.map((id) => vavAmneById(id)?.kod).join(", ")}
            </span>
          )}
        </div>
      </form>
        ...
        <section className="mt-6">
          ...
          <div className="flex flex-wrap gap-2">
            {senaste.map((s, i) => (
              ...
              >
                {s.tema} ·{" "}
                {s.amnen.map((id) => vavAmneById(id)?.kod).join("/")}
              </button>
            ))}
          </div>
        </section>
      ...
      <footer className="mt-14 border-t border-ram pt-6 text-sm text-dampad">
        <p>
          Källa: Skolverkets öppna data (Syllabus API), Lgr22. Förslagen
          genereras med AI och är ett stöd – läraren ansvarar för den
          pedagogiska bedömningen. Inga personuppgifter om elever hanteras.
        </p>
      </footer>
    </main>
  );
