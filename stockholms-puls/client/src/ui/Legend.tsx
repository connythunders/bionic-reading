    ...
    <div ref={drag.ref as any} className="legend panel" style={drag.style} {...drag.handlers}>
      ...
      {!collapsed && groups.map((g) => {
        ...
        return (
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 4px", width: "100%", background: "transparent", border: "none", color: isOn ? "var(--ink)" : "#5a697f", cursor: "pointer", fontSize: 12, textAlign: "left", fontFamily: "inherit", borderRadius: 4, transition: "color 0.15s" }}
          >
            <Toggle on={isOn} color={g.color} />
            <span style={{ width: 10, height: 10, borderRadius: 2, background: isOn ? g.color : `${g.color}40`, boxShadow: isOn ? `0 0 6px ${g.color}` : "none", flexShrink: 0 }} />
      ...
      {!collapsed && (
        <>
          ...
          <button onClick={() => toggleMode("bus")} style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 4px", width: "100%", background: "transparent", border: "none", color: busOn ? "var(--ink)" : "#5a697f", cursor: "pointer", fontSize: 12, textAlign: "left", fontFamily: "inherit", borderRadius: 4 }}>
          ...
          <button onClick={() => setShowBasemap(!showBasemap)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 4px", width: "100%", background: "transparent", border: "none", color: showBasemap ? "var(--ink)" : "#5a697f", cursor: "pointer", fontSize: 12, textAlign: "left", fontFamily: "inherit", borderRadius: 4 }}>
          ...
          <div style={{ height: 10 }} />
          <div style={{ fontSize: 10, color: "#8b98ad", letterSpacing: 0.08, textTransform: "uppercase", marginBottom: 4 }}>Kronisk försening (senaste ~24h)</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
            <div style={{ flex: 1, height: 6, borderRadius: 3, background: "linear-gradient(90deg, #4bd582, #ffc04a, #ff3030)", opacity: 0.85 }} />
            <span style={{ fontSize: 10, color: "#8b98ad", minWidth: 54, textAlign: "right" }}>sällan → ofta</span>
          </div>
          <div style={{ fontSize: 10.5, color: "#8b98ad", lineHeight: 1.4 }}>Klicka i listan för att visa/dölja linjer. Byt region i toppen.</div>
        </>
      )}
    </div>
  );
...
function Toggle({ on, color }: { on: boolean; color: string }) {
  return (
    <span style={{ width: 26, height: 14, borderRadius: 7, background: on ? color : "rgba(255,255,255,0.14)", position: "relative", transition: "background 0.18s", flexShrink: 0, boxShadow: on ? `0 0 8px ${color}70` : "none" }}>
      <span style={{ position: "absolute", top: 2, left: on ? 14 : 2, width: 10, height: 10, borderRadius: "50%", background: "#fff", transition: "left 0.18s", boxShadow: "0 1px 2px rgba(0,0,0,0.4)" }} />
    </span>
  );
}
