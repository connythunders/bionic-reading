...
export function CityBase({ projection }: { projection: Projection }) {
  const waterOutlines = useMemo(() => {
    return [RIDDARFJARDEN_LATLON, MALAREN_LATLON, SALTSJON_LATLON, BRUNNSVIKEN_LATLON].map((pts) => {
      ...
      const mat = new THREE.LineBasicMaterial({ color: "#4ca5e8", transparent: true, opacity: 0.55 });
      const obj = new THREE.Line(geo, mat);
      obj.position.set(0, 0.002, 0);
      return obj;
    });
  }, [projection]);
  ...
  return (
    <group>
      ...
      {waterOutlines.map((obj, i) => (
        <primitive key={`wl-${i}`} object={obj} />
      ))}
...
function MajorGrid() {
  const grid = useMemo(() => {
    ...
    const mat = new THREE.LineBasicMaterial({ color: "#24406b", transparent: true, opacity: 0.55, depthWrite: false });
...
function MinorGrid() {
  const grid = useMemo(() => {
    ...
    geo.setAttribute("position", new THREE.Float32BufferAttribute(verts, 3));
    const mat = new THREE.LineBasicMaterial({ color: "#0f1d33", transparent: true, opacity: 0.35, depthWrite: false });
    return new THREE.LineSegments(geo, mat);
  }, []);
...
function useStarField(count: number) {
  return useMemo(() => {
    ...
    const mat = new THREE.PointsMaterial({ size: 0.18, sizeAttenuation: true, vertexColors: true, transparent: true, opacity: 0.7, depthWrite: false, blending: THREE.AdditiveBlending });
...
function useDustField(count: number) {
  return useMemo(() => {
    ...
    geo.setAttribute("position", new THREE.Float32BufferAttribute(pos, 3));
    const mat = new THREE.PointsMaterial({ size: 0.04, color: "#9bc4ff", transparent: true, opacity: 0.35, depthWrite: false, sizeAttenuation: true, blending: THREE.AdditiveBlending });
    return new THREE.Points(geo, mat);
  }, [count]);
}
